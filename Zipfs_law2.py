...
word_count_pair = [(k, v) for k, v in word_count_dict.items()]
word_count_pair.sort(reverse=True, key = lambda x:x[1])
word_rank_list = [pair[0] for pair in word_count_pair]
...
for i in range(len(word_count_dict)):
    ...
    if i > 0 and word_count_dict[word_rank_list[i-1]] == freq:
        pass
    else:
        rank = i + 1
